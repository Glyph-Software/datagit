...
func Plan(tableID uint64, physical string, changes []Change) *adapter.MigrationPlan {
	...
	for _, c := range changes {
		switch c.Kind {
		case AddColumn:
			add(string(c.Kind), fmt.Sprintf(
				`ALTER TABLE %s ADD COLUMN IF NOT EXISTS %s %s`,
				q(physical), q(c.To.Name), c.To.SQLType), c.Class)
		case DropNotNull:
			add(string(c.Kind), fmt.Sprintf(
				`ALTER TABLE %s ALTER COLUMN %s DROP NOT NULL`,
				q(physical), q(c.To.Name)), c.Class)
		...
		case SetNotNull:
			...
			add("preflight_not_null", fmt.Sprintf(
		...
		case RenameColumn:
			add(string(c.Kind), fmt.Sprintf(
				...
				q(physical), q(c.From.Name), q(c.To.Name)), c.Class)
		case DropColumn:
			// Two-phase (§10.4). Phase one makes the column unwritable and
			// invisible to DataGit while leaving it readable, so a deploy can be
			// sequenced around it; phase two drops it after the window.
			add("deprecate_column", fmt.Sprintf(
				`COMMENT ON COLUMN %s.%s IS 'datagit: deprecated, pending drop'`,
				q(physical), q(c.From.Name)), c.Class)
			add(string(c.Kind), fmt.Sprintf(
